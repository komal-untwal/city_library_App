			...
			public void actionPerformed(ActionEvent e) {
				...
				if (reservedResult == null || reservedResult.size() <= 0) {
					...
					if (borrowResult == null || borrowResult.size() <= 0) {
						...
						/*
						 * ArrayList<ArrayList<Object>> readerReserved = new
						 * ArrayList<ArrayList<Object>>(); readerReserved =
						 * m.execQuery("SELECT * FROM RESERVES WHERE `RID` = '" +  + "';");
						 * Object[][] readerReservedArr = new Object[readerReserved.size()][]; for (int
						 * i = 0; i < readerReserved.size(); i++) { ArrayList<Object> row =
						 * readerReserved.get(i); readerReservedArr[i] = row.toArray(); } Calendar cl1 =
						 * Calendar.getInstance(TimeZone.getDefault()); int year1 =
						 * cl1.get(Calendar.YEAR); int month1 = cl1.get(Calendar.MONTH); int day1 =
						 * cl1.get(Calendar.DAY_OF_MONTH); cl1.set(year1, month1, day1, 18, 0, 0); //
						 * Today's 6pm for (int q = 0; q < readerReservedArr.length; q++) { long dtime =
						 * ((Timestamp)readerReservedArr[q][5]).getTime() + 86400000; // Reserved time +
						 * 1 day // If reserved after yesterday 6pm. Then check if now is after 6pm. if
						 * (cl1.getTimeInMillis() - dtime < 0) { // If now is after 6pm, Check if
						 * reserved after today 6pm. if ((System.currentTimeMillis() -
						 * cl1.getTimeInMillis()) > 0) { // If now is after 6m and reserved before today
						 * 6pm. It is expired. if ((dtime - 86400000) - cl1.getTimeInMillis() < 0) { int
						 * deletedRows = m.execUpdate("DELETE FROM RESERVES WHERE `DTIME` = '" +
						 * readerReservedArr[q][5] + "';"); if (deletedRows != 1) {
						 * JOptionPane.showMessageDialog(null, "Failed to remove reservation"); return;
						 * } } // else if reserved after today 6pm, it isn't expired. don't remove. move
						 * to next tuple. }// If now is before 6pm, it isn't expired. don't remove. move
						 * to next tuple. } else { // If reserved before yesterday 6pm, then it is
						 * expired, should remove. int deletedRows =
						 * m.execUpdate("DELETE FROM RESERVES WHERE `DTIME` = '" +
						 * readerReservedArr[q][5] + "';"); if (deletedRows != 1) {
						 * JOptionPane.showMessageDialog(null, "Failed to remove reservation"); return;
						 * } } }
						 */
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (reservedResult == null || reservedResult.size() <= 0) {
					...
					if (borrowResult == null || borrowResult.size() <= 0) {
						 ...
						 * ArrayList<ArrayList<Object>> readerReserved = new
						 * ArrayList<ArrayList<Object>>(); readerReserved =
						 * m.execQuery("SELECT * FROM RESERVES WHERE `RID` = '" + readerId + "';");
						 * Object[][] readerReservedArr = new Object[readerReserved.size()][]; for (int
						 * i = 0; i < readerReserved.size(); i++) { ArrayList<Object> row =
						 * readerReserved.get(i); readerReservedArr[i] = row.toArray(); } Calendar cl1 =
						 * Calendar.getInstance(TimeZone.getDefault()); int year1 =
						 * cl1.get(Calendar.YEAR); int month1 = cl1.get(Calendar.MONTH); int day1 =
						 * cl1.get(Calendar.DAY_OF_MONTH); cl1.set(year1, month1, day1, 18, 0, 0); for
						 * (int q = 0; q < readerReservedArr.length; q++) { long dtime =
						 * ((Timestamp)readerReservedArr[q][5]).getTime() + 86400000; // Reserved time +
						 * 1 day // If reserved after yesterday 6pm. Then check if now is after 6pm. if
						 * (cl1.getTimeInMillis() - dtime < 0) { // If now is after 6pm, check if
						 * reserved after today 6pm. if ((System.currentTimeMillis() -
						 ...
						 * JOptionPane.showMessageDialog(null, "Failed to remove reservation"); return;
						 * } } // else if reserved after today 6pm, it isn't expired. don't remove. move
						 * to next tuple. }// If now is before 6pm, don't remove. move to next tuple. }
						 * else { // If reserved before yesterday 6pm, then it is expired, should
						 * remove. int deletedRows =
						 * m.execUpdate("DELETE FROM RESERVES WHERE `DTIME` = '" +
						 * readerReservedArr[q][5] + "';"); if (deletedRows != 1) {
						 * JOptionPane.showMessageDialog(null, "Failed to remove reservation"); return;
						 * } } }
